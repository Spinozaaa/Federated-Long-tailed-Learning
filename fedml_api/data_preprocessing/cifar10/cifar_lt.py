...
class IMBALANCECIFAR10(torchvision.datasets.CIFAR10):
    ...
    def __init__(self, imbalance_ratio=1, root='../data', dataidxs=None, train=True, imb_type='exp',
        ...
        if self.train:
            ...
            if dataidxs is None:
                ...
                self.gen_imbalanced_data(self.img_num_list)
                self.__build_truncated_dataset__(dataidxs)
        else:
            self.transform = transform
    ...
    def __build_truncated_dataset__(self, dataidxs):
        self.data = np.array(self.data[dataidxs])
    ...
    def __getitem__(self, index):
        ...
        if self.contrast:
            ...
            if self.transform is not None:
                ...
                sample3 = self.transform[2](img)  # for training_data test
    ...
    def gen_imbalanced_data(self, img_num_per_cls):
        new_data = []
        new_target = []
        target_np = np.array(self.targets, dtype=np.int64)
        ...
        for the_class, the_img_num in zip(classes, img_num_per_cls):
            ...
            idx = np.where(target_np == the_class)[0]
            # np.random.shuffle(idx)
            selec_idx = idx[:the_img_num]
            new_data.append(self.data[selec_idx, ...])
            new_target.extend([the_class, ] * the_img_num)
...
class SMALLCIFAR10(torchvision.datasets.CIFAR10):
    ...
    def gen_imbalanced_data(self, img_num_per_cls):
        new_data = []
        new_target = []
        target_np = np.array(self.target, dtype=np.int64)
        classes = np.unique(target_np)
        self.num_per_cls_dict = dict()
        for the_class, the_img_num in zip(classes, img_num_per_cls):
            # print(the_img_num, end=' ')
