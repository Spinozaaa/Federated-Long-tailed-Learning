...
def get_dataloader_ImageNet_truncated(imagenet_dataset_train: ImageNet, imagenet_dataset_test: ImageNet, train_bs,
                                      test_bs, dataidxs=None, net_dataidx_map=None, args=None, get_bala_dataloader=False):

    dl_obj = ImageNet_truncated
    if args is not None and "paco" in args.method:
        transform_train, transform_test = paco_aug(dataset=args.dataset, aug="imagenet")
    else:
    ...
    test_ds = dl_obj(imagenet_dataset_test, dataidxs=None, net_dataidx_map=None, train=False, transform=transform_test,
                     download=False)
    if not get_bala_dataloader:
        train_dl = data.DataLoader(dataset=train_ds, batch_size=train_bs, shuffle=True, drop_last=False)
    else:
        train_dl = data.DataLoader(dataset=train_ds, batch_size=train_bs, shuffle=False, sampler=ClassAwareSampler(train_ds, 4, 1000, train_bs), drop_last=False)
...
def load_partition_data_ImageNet_raw(args, get_bala_dataloader=False):
    ...
    if not get_bala_dataloader:
        ...
        logging.info("##########################Get balance dataset##########################")
        for client_idx in range(args.client_num_in_total):
            ...
            train_data_local, test_data_local = get_dataloader_ImageNet_truncated(train_dataset, test_dataset,
                                                                                  train_bs=args.batch_size, test_bs=args.batch_size,
                                                                                  dataidxs=client_idx, net_dataidx_map=net_dataidx_map,
                                                                                  args=args, get_bala_dataloader=True)
